...
async function getVerdict(application) {
  const prompt = `
    ...
    Кандидаты, которые имеют наибольший приоритет:
...
async function fineTuneModel(application, mentorVerdict, mentorExplanation) {
  // Для GPT-3.5-turbo-16k мы не можем выполнить настоящий fine-tuning,
  // но мы можем использовать эту информацию для улучшения промпта
  // Здесь мы можем сохранять эту информацию в базу данных или файл
  // для последующего анализа и улучшения промпта
  const trainingData = {
    application,
    mentorVerdict,
    mentorExplanation,
  ...
  // Здесь должна быть логика сохранения trainingData
  console.log('New training data:', trainingData);

  // В реальном приложении здесь можно было бы периодически анализировать
  // накопленные данные и автоматически обновлять промпт
}

module.exports = { getVerdict, fineTuneModel };